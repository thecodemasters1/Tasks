...
//derived CalculatorOperation Subtraction: Subtracts the parameters from the current value;
class OpSubtract : public CalculatorOperation
{
public:
	//C'tor, Get vector with keywords of the operation Set supported keywords for the operation (use base abstractt c'tor).
	OpSubtract(const vector<string>& operations) : CalculatorOperation(operations) {}
	//D'tor
	virtual ~OpSubtract() {}
